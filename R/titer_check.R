...
thresholds.from.triton = function(ffp,
                                  folderID = NA,
                                  fn = NA,
                                  parameters = 7:16,
                                  defaultDesc = c(
  ...
  if (all(is.na(parameters(ffp$ffOrig)$desc))) {
    names(detectors) <- defaultCols[nona_parameters]
  } else{
  ...
  for (i in 1:length(nona_parameters)) {
    ...
    kde[[i]] = KernSmooth::bkde(exprs(tmp$ffTxed)[, pname],
                                bandwidth = 0.02,
                                gridsize = 4001)
  ...
  for (i in 1:length(nona_parameters)) {
    ...
    pgate[[i]] = flowCore::rectangleGate(filterId = paste(p, "+", sep = ""),
                                         .gate = eval(parse(text = gexpr)))
    ngexpr = paste("list('", p, "'=c(-Inf, ", thresh[i], "))", sep = "")
    ngate[[i]] = flowCore::rectangleGate(filterId = paste(p, "-", sep = ""),
                                         .gate = eval(parse(text = ngexpr)))
  }
...
plot_all_conjugates_vs_ssc <-
  function(ffp,
    ...
    nona_parameters <- which(!is.na(parameters(ffp$ffOrig)$desc))
    p = colnames(ffp$ffOrig)[nona_parameters]

    stats <- thresholds.from.triton(ffp)
    thresh <- stats[['thresh']]
    kde <- stats[['kde']]
    gde <- stats[['gde']]
    ffg = Subset(ffp$ffOrig, flowCore::rectangleGate("SSC-A" = c(-Inf, size.thresh)))
    ...
    for (i in 1:length(nona_parameters)) {
      ...
      plottitle = paste(colnames(ffp$ffOrig)[nona_parameters[i]],
                        parameters(ffp$ffOrig)[nona_parameters[i]]$desc)
      ffp$plot(
        plist = c(p, "SSC-A"),
        xlim = c(-.5, flowFramePlus:::bx(5000)),
        main = plottitle
      )
      left.edge = -2
      polygon(
        x = c(left.edge, thresh[i],
              thresh[i],
              left.edge),
        y = c(0, 0, 5.4, 5.4),
        col = '#0000000F'
      ...
      text(
        flowFramePlus:::bx(300),
        flowFramePlus:::bx(300),
        labels = sprintf(
          "fp = %d (%.3f%%)",
          res_fp$n_fp[i],
          100 * res_fp$n_fp[i] / res_fp$n_total
        ),
        pos = 4,
        cex = 2
      )
